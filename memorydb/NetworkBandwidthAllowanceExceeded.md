---
Title: MemoryDB 'NetworkBandwidthAllowanceExceeded' alarm
Author: Pranav
Date: 2026/01/31
---

# MemoryDB *NetworkBandwidthAllowanceExceeded* alarm

## Objective
* Detect when MemoryDB nodes exceed their allocated network bandwidth, which can lead to throttling.
...
## Overview
For example, a `db.t4g.small` instance type has up to **5 Gigabit(Gbps)** of network bandwidth, which apply to both inbound and outbound traffic of the instance. 
...
AWS [network specifications](https://docs.aws.amazon.com/ec2/latest/instancetypes/gp.html#gp_network) page shows that the `Baseline / Burst bandwidth (Gbps)` for the EC2 instance type of **t4g.small** as `0.128 / 5.0`, these specifications applies to MemoryDB nodes as well since they're built on the same EC2 instance types.
- **0.128Gbps(~128Mbps)** is the baseline guaranteed network bandwidth of the instance.
    Even if the host is under heavy load, the instance should get at least 128Mbps for the traffic.
...
Outgoing traffic can be response to clients, replication updates sent from the primary node to replicas etc,.
...
Let's set the `period` to **60 seconds**, with `evaluation_periods` and `datapoints_to_alarm` around **15 minutes** to avoid false positives.
...
## References
1. https://docs.aws.amazon.com/memorydb/latest/devguide/metrics.HostLevel.html
2. https://aws.amazon.com/blogs/database/monitor-server-side-latency-for-amazon-memorydb-for-valkey/
3. https://aws.amazon.com/memorydb/features/#topic-1
